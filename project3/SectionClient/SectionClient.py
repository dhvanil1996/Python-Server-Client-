...
def main(filename, *addresses):
    ...
    with selectors.DefaultSelector() as sel:
        ...
        while(run):
            for key, event in sel.select(timeout = 0):
                sock = key.fileobj
                sec_data = key.data
                if event & selectors.EVENT_READ:
                    recv_data = sock.recv(SIZE_1_KiB)
                    while recv_data:
                        sec_data.data.extend(recv_data)
                        recv_data = sock.recv(SIZE_1_KiB)
                    if(sec_data.check_integrity()):
                        file_contents[sec_data.from_byte : sec_data.to_byte] = sec_data.data
                        sel.unregister(sock)
                        sock.close()
                    run = not(len(file_contents) == total_size)
                if event & selectors.EVENT_WRITE:
                    if(sec_data.request_sent == False):
                        msg = f'SECTION {sec_data.num}'
                        sock.send(msg.encode())
                        sec_data.request_sent = True
